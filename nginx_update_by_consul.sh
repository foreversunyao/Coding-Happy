...
#cp $nginx_path/conf.d/$1.conf $bak_nginx_path/bk.$1.conf.$timestamp"
#consul-template -dry -consul=$consul_instance -log-level=ERR -syslog -once -template "$bak_nginx_path/$tpl_file:$nginx_path/conf.d/$1.conf:echo 1"
}
...
#consul-template -consul=$consul_instance -log-level=ERR -syslog -once -template "$tpl_file:conf.d/$1.conf:echo $timestamp >>test.log"
}
main ()
{
  if [ $? -eq 0 ];then
